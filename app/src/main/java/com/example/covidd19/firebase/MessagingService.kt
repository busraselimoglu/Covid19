package com.example.covidd19.firebase

import android.content.Intent
import android.util.Log
import androidx.localbroadcastmanager.content.LocalBroadcastManager
import com.example.covidd19.activities.IncomingInvitationActivity
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (type != null) {
            if (type == Constants.REMOTE_MSG_INVITATION) {
                ...
                intent.putExtra(
                    Constants.KEY_ID,
                    remoteMessage.data[Constants.KEY_ID]
                )
                intent.putExtra(
                    Constants.KEY_OTHER,
                    remoteMessage.data[Constants.KEY_OTHER]
                )
                intent.putExtra(
